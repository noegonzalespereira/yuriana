    ...
    <div className="w-full overflow-x-auto rounded-2xl border border-border">
      <table className="w-full text-left border-collapse">
        <thead className="bg-[var(--yuriana-base-orange)] text-white uppercase text-[10px] font-black tracking-widest">
          <tr>
            <th className="px-6 py-4">N° Placa</th>
            <th className="px-6 py-4">Tipo de Unidad</th>
            <th className="px-6 py-4">Detalles Mecánicos</th>
        ...
        <tbody className="divide-y divide-border bg-[var(--yuriana-base-white)] font-medium text-gray-700">
          {data.length === 0 ? (
            ...
            data.map((item) => (
              <tr key={item.id_unidad} className="hover:bg-slate-50/80 transition-colors text-sm">
                {/* PLACA */}
                <td className="px-6 py-4 font-black text-gray-900 tracking-tight uppercase">
                  {item.placa}
                </td>
