...
import { PlusCircle, Trash2, Info, Layers } from "lucide-react";
import { toast } from "sonner";
import { TipoGastoGeneral, GastoGeneral, ItemGastoForm } from "@/types/gasto.types";
import { guardarGastoGeneral, editarGastoGeneral } from "@/lib/api/gasto.api";

const TIPOS_GASTO = Object.values(TipoGastoGeneral);

const TIPO_COLOR: Record<string, string> = {
  [TipoGastoGeneral.TALLER]:  "text-amber-500",
  [TipoGastoGeneral.LLANTAS]: "text-blue-500",
  [TipoGastoGeneral.OTROS]:   "text-slate-500",
};
...
interface Props {
  initialData?: GastoGeneral | null;
  isReadOnly?: boolean;
  onCancel: () => void;
  onSuccess: () => void;
}

export const GastoGeneralForm = ({ initialData, isReadOnly = false, onCancel, onSuccess }: Props) => {
  const isEdit = !!initialData;

  const [items, setItems] = useState<ItemGastoForm[]>([]);
  const [newFecha, setNewFecha] = useState("");
  const [newTipo, setNewTipo] = useState<string>(TipoGastoGeneral.TALLER);
  ...
  useEffect(() => {
    if (initialData) {
      setItems([{
        _key: String(initialData.id_gasto_general),
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEdit && initialData) {
        const item = items[0];
        await editarGastoGeneral(initialData.id_gasto_general, {
        ...
        toast.success("Gastos generales registrados correctamente.");
      }
      onSuccess();
    } catch (err: any) {
      ...
      <div className="bg-[var(--yuriana-card-bg)] rounded-3xl border border-border shadow-xl p-8 space-y-6">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-xl font-black text-[var(--yuriana-base-gray-dark)] uppercase tracking-tight">
            {isEdit ? "Editar Gasto General" : "Registrar Gastos Generales"}
          </h2>
        </div>
        <SectionHeader icon={<Layers size={16} />} title="Egresos generales de la empresa" />
      </div>
      ...
      <div className="flex gap-6 items-start">
        <div className="flex-1 bg-[var(--yuriana-card-bg)] rounded-3xl border border-border shadow-xl overflow-hidden">
          ...
          <div className="p-6 space-y-4">
            {!isReadOnly && (
              <div className="grid grid-cols-[130px_160px_1fr_120px_auto] gap-3 items-end">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <input type="text" value={newDescripcion} onChange={(e) => setNewDescripcion(e.target.value)}
                    placeholder="Ej: Cambio de llanta delantera" className={INPUT_CLASS} />
                </div>
                <div className="flex flex-col gap-1">
