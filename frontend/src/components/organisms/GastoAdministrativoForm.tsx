...
import { TipoGastoAdministrativo, GastoAdministrativo, ItemGastoForm } from "@/types/gasto.types";
import { guardarGastoAdministrativo, editarGastoAdministrativo } from "@/lib/api/gasto.api";

const TIPOS_GASTO = Object.values(TipoGastoAdministrativo);

const TIPO_COLOR: Record<string, string> = {
  [TipoGastoAdministrativo.CONTADOR]: "text-purple-500",
  [TipoGastoAdministrativo.IMPUESTO]: "text-red-500",
  [TipoGastoAdministrativo.GPS]: "text-blue-500",
  [TipoGastoAdministrativo.SUELDO_CONDUCTORES]: "text-emerald-500",
  [TipoGastoAdministrativo.OTROS]: "text-slate-500",
};

interface Props {
  initialData?: GastoAdministrativo | null;
  isReadOnly?: boolean;
  onCancel: () => void;
  onSuccess: () => void;
}
...
export const GastoAdministrativoForm = ({ initialData, isReadOnly = false, onCancel, onSuccess }: Props) => {
  const isEdit = !!initialData;

  const [items, setItems] = useState<ItemGastoForm[]>([]);
  const [newFecha, setNewFecha] = useState("");
  const [newTipo, setNewTipo] = useState<string>(TipoGastoAdministrativo.CONTADOR);
  ...
  useEffect(() => {
    if (initialData) {
      setItems([{
        _key: String(initialData.id_gasto_admin),
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEdit && initialData) {
        ...
        toast.success("Gastos administrativos registrados correctamente.");
      }
      onSuccess();
    } catch (err: any) {
      ...
      <div className="bg-[var(--yuriana-card-bg)] rounded-3xl border border-border shadow-xl p-8 space-y-6">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-xl font-black text-[var(--yuriana-base-gray-dark)] uppercase tracking-tight">
            {isEdit ? "Editar Gasto Administrativo" : "Registrar Gastos Administrativos"}
          </h2>
        </div>
        <SectionHeader icon={<Building2 size={16} />} title="Egresos de la empresa" />
      </div>

      {/* Tabla de ítems + panel de totales */}
      <div className="flex gap-6 items-start">
        <div className="flex-1 bg-[var(--yuriana-card-bg)] rounded-3xl border border-border shadow-xl overflow-hidden">
          ...
          <div className="p-6 space-y-4">
            {!isReadOnly && (
              <div className="grid grid-cols-[130px_170px_1fr_120px_auto] gap-3 items-end">
                <div className="flex flex-col gap-1">
                  <label className="text-[9px] font-black uppercase tracking-widest text-[var(--yuriana-input-label)] ml-1">Fecha</label>
                  <input type="date" value={newFecha} onChange={(e) => setNewFecha(e.target.value)} className={INPUT_CLASS} />
                </div>
                <div className="flex flex-col gap-1">
                  <label className="text-[9px] font-black uppercase tracking-widest text-[var(--yuriana-input-label)] ml-1">Tipo de Gasto</label>
                  <select value={newTipo} onChange={(e) => setNewTipo(e.target.value)} className={`${INPUT_CLASS} capitalize`}>
                    {TIPOS_GASTO.map((t) => (
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <input type="text" value={newDescripcion} onChange={(e) => setNewDescripcion(e.target.value)}
                    placeholder="Ej: Pago contador enero" className={INPUT_CLASS} />
                </div>
                <div className="flex flex-col gap-1">
                  <label className="text-[9px] font-black uppercase tracking-widest text-[var(--yuriana-input-label)] ml-1">Monto Bs</label>
                  <input type="number" min={0} step="any" value={newMonto || ""} onChange={(e) => setNewMonto(Number(e.target.value))}
                    ...
                    items.map((item) => (
                      <tr key={item._key} className="hover:bg-orange-50/30 transition-colors">
                        ...
                        <td className="px-4 py-2.5">
                          <span className="font-black text-[10px] uppercase text-[var(--yuriana-base-orange)]">
                            {item.tipo_gasto.replace("_", " ")}
                          </span>
                        </td>
                        <td className="px-4 py-2.5 text-[var(--yuriana-base-gray-dark)] font-medium">{item.descripcion}</td>
                        <td className="px-4 py-2.5 text-right font-black text-[var(--yuriana-base-gray-dark)]">{fmt(item.monto)}</td>
                        {!isReadOnly && (
          ...
          <div className="space-y-3">
            {TIPOS_GASTO.map((tipo) =>
              totalesPorTipo[tipo] > 0 ? (
                <div key={tipo}>
                  <p className={`text-[10px] uppercase font-black tracking-wider ${TIPO_COLOR[tipo] ?? "text-gray-500"}`}>
                    {tipo.replace("_", " ")}
                  </p>
