    ...
    <div className="w-full overflow-hidden rounded-3xl border border-border shadow-sm">
      <table className="w-full text-left border-collapse">
        <thead className="bg-yuriana-orange text-white uppercase text-[10px] font-black tracking-widest">
          <tr>
            <th className="px-4 py-2.5">Nombre del Documento</th>
            <th className="px-4 py-2.5 text-center">Es Obligatorio</th>
            <th className="px-4 py-2.5 text-center">Requiere Vencimiento</th>
          ...
          {data.length === 0 ? (
            ...
            data.map((item) => (
              <tr key={item.id_requisito_documento} className="hover:bg-slate-50/80 transition-colors">
                <td className="px-4 py-2.5 text-xs font-bold flex items-center gap-3 text-gray-800">
